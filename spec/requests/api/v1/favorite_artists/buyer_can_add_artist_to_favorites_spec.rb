...
describe 'POST /api/v1/favorite_artists' do
  context 'a buyer favoriting an artist' do
    ...
    it 'will be return a message if the artist is already favorited' do
      @buyer.favorite_artists.create(artist_id: @artist.id)
      ...
      expect(new_favorite_artist[:data][:buyer_id]).to eq(@buyer.id)
      expect(new_favorite_artist[:data][:artist_id]).to eq(@artist.id)
      expect(new_favorite_artist[:message]).to eq("#{@artist.name} is already in favorites")
    end
